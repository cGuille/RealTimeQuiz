var app = require("http").createServer(handler)
  ...
  , CheckingFactory = require("./checkingfactory").CheckingFactory

  , lib_dir = process.cwd() + "/lib"
  , static_dir =  process.cwd() + "/static"
  , port = 8080

  , cf = new CheckingFactory()
...
io.sockets.on("connection", function (socket) {
  ...
  socket.on("answer", function (data) {
    console.log("Question: " + quiz.questions[data.id].question);
    console.log("Input answer: " + data.answer);
    console.log("Answer command: " + quiz.questions[data.id].answer);
    console.log("Result: ", cf.getChecker(quiz.questions[data.id].answer)(data.answer));
  });
});
